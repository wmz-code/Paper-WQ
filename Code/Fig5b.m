...
uplim=1000;lowlim=0;
load('D:\projection\all585.mat'); 
condition_names = {'BWk', 'BSk', 'Cwa', 'Cwb','Cfa','Dwa','Dwb','Dwc'};
% plot for mean of 135 models under ssp585
for mm=2 %model
    for qq=1:8 %indicators
        ...
        for tt=1:3 %tem
            for yy=1:3 %period
                for pp=1:3 %stages
                    path=strcat(['D:\Robustness Checks\',model{mm},'\',cli{tt},'\',years{yy},'\P585_',emodel{mm},'_random_phe',num2str(pp),'_d.xlsx']);
                    name=quality{qq};
...
% print -dpdf -r600 -painters Fig5_violin
