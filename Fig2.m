...
for ii=1:8 % climate zone
    cc=clizone(ii);
    path=strcat(['/data/Estimation/Model_bayes_clizone',num2str(cc),'.xlsx']);
    Tdata=xlsread(path,'et');
    Tdata=Tdata.*100;
    q1(ii,:)=Tdata(1,:); % Change the order
    q2(ii,:)=Tdata(5,:);
    q3(ii,:)=Tdata(2,:);
    q4(ii,:)=Tdata(4,:);
    q5(ii,:)=Tdata(3,:);
...
tm=1;
set(gcf,'Position',[698 19 1825 1311]);
...
xxx=[1:1:8];
aaa=1;
locations=[1 2 5 6 9 10 13 14];
for jj=1:8
    eval(['qq=',['q',num2str(jj)]]);
    subplot(4,4,locations(jj));
    h=barh(1,qq(1,1));
...
aa=[-9 9;-6 6;-9 9;-9 9;-21 21;-66 66;-66 66;-66 66];
gap=[-9:3:9;-6:2:6;-9:3:9;-9:3:9;-21:7:21;-66:22:66;-66:22:66;-66:22:66];
xx=[1:1:8];
xxx=[1:1:8];
locations=[3 4 7 8 11 12 15 16];
for jj=1:8
    eval(['qq=',['q',num2str(jj)]]);
    subplot(4,4,locations(jj));
    h=barh(1,qq(1,1));
...
print(gcf, '/results/fig2.png', '-dpng', '-r300');
